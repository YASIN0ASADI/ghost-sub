#!/usr/bin/env python3
"""
Ghost Sub — Usage Tracker (Marzban / Pasarguard)
──────────────────────────────────────────────────
هر N دقیقه با API پنل (Marzban یا Pasarguard) لاگین می‌کنه، مصرف تمام
یوزرها رو می‌خونه و یه تاریخچه‌ی سبک (snapshot) کنار فایل تمپلیت ذخیره می‌کنه.

Marzban و Pasarguard دقیقاً همون ساختار API رو دارن (چون Pasarguard فورک
Marzban هست): POST /api/admin/token و GET /api/users با pagination یکسان.
پس یک اسکریپت برای هر دو پنل کار می‌کنه.
...
تمپلیت (marzban.html یا pasarguard.html) این فایل رو fetch می‌کنه و نمودار مصرف رو می‌کشه.
"""
...
def load_config():
    ...
    cfg.setdefault("panel_type", "marzban")  # "marzban" یا "pasarguard" — فقط برای لاگ، API یکسانه
    return cfg
...
def main():
    ...
    log.info(f"Ghost Tracker started. Panel: {cfg.get('panel_type','marzban')} @ {cfg['panel_url']}  Interval: {cfg['interval_minutes']}m")
