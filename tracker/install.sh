...
echo -e "  ${DIM}(اگه لینک جداگانه نمی‌خوای، Enter بزن تا همون چنل تلگرام استفاده بشه)${RST}"
read -rp "  ➤ Renew URL (leave empty = use channel): " RENEW_RAW
RENEW_RAW=$(echo "$RENEW_RAW" | xargs 2>/dev/null || echo "$RENEW_RAW")
if [[ -z "$RENEW_RAW" ]]; then
  RENEW_URL="$TG_URL"
else
...
# ── Optional: Usage Chart Tracker (Pasarguard only) ────────────────
if [[ "$PANEL_CHOICE" == "2" ]]; then
  ...
  if [[ "$INSTALL_TRACKER" =~ ^[Yy]$ ]]; then
    ...
    if [[ -n "$TRACKER_PANEL_URL" && -n "$TRACKER_ADMIN_USER" && -n "$TRACKER_ADMIN_PASS" ]]; then
      ...
      # ساخت فایل کانفیگ
      cat > "$TRACKER_DIR/tracker_config.json" << CFGEOF
{
...
CFGEOF
      chmod 600 "$TRACKER_DIR/tracker_config.json"
